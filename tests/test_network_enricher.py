...
def test_get_public_ip_caches_result():
    ...
    mock_response = MagicMock()
    mock_response.json.return_value = {"ip": "203.0.113.1"}
    with patch("visionlog.enrichers.network.httpx.get", return_value=mock_response) as mock_get:
        first = get_public_ip()
        second = get_public_ip()
    assert first == second == "203.0.113.1"
    # httpx.get should only have been called once thanks to lru_cache
...
def test_get_geo_info_missing_fields_default_to_empty():
    ...
    mock_response = MagicMock()
    mock_response.json.return_value = {"city": "London"}
    with patch("visionlog.enrichers.network.httpx.get", return_value=mock_response):
        result = get_geo_info("203.0.113.1")
...
def test_get_geo_info_uses_timeout_parameter():
    """Verifies that get_geo_info passes the timeout argument to httpx.get."""
    mock_response = MagicMock()
    mock_response.json.return_value = {}
    with patch("visionlog.enrichers.network.httpx.get", return_value=mock_response) as mock_get:
...
def test_network_enricher_ip_and_geo_binds_both():
    ...
    ip_response.json.return_value = {"ip": "10.0.0.2"}
    ...
    responses = [ip_response, geo_response]

    with patch("visionlog.enrichers.network.httpx.get", side_effect=responses):
        logger = get_logger()
...
def test_network_enricher_geo_false_skips_geo():
    ...
    get_public_ip.cache_clear()
    mock_response = MagicMock()
    mock_response.json.return_value = {"ip": "10.0.0.3"}
    with patch("visionlog.enrichers.network.httpx.get", return_value=mock_response) as mock_get:
        logger = get_logger()
    ...
    assert "city" not in logger._context
    # httpx.get should only have been called once (for IP, not geo)
    mock_get.assert_called_once()
...
def test_network_enricher_can_be_used_with_get_logger_enrichers():
    ...
    get_public_ip.cache_clear()
    mock_response = MagicMock()
    mock_response.json.return_value = {"ip": "10.0.0.4"}
    with patch("visionlog.enrichers.network.httpx.get", return_value=mock_response):
        logger = get_logger(
            user_id="u99",
            privacy_mode=False,
